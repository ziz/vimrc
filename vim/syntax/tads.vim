...
syn keyword tadsKeyword		operator method invokee defined
...
syn region tadsTrebleDoubleString		start=+\z("\{3,}\)+ skip=+\\\\\|\\\z1+ end=+\z1+ contains=tadsSpecial,tadsEmbedded
syn region tadsTrebleSingleString		start=+\z('\{3,}\)+ skip=+\\\\\|\\\z1+ end=+\z1+ contains=tadsSpecial,tadsEmbedded
syn region tadsDoubleString		start=+"\(""\)\@!+ skip=+\\\\\|\\"+ end=+"+ contains=tadsSpecial,tadsEmbedded
syn region tadsSingleString		start=+'\(''\)\@!+ skip=+\\\\\|\\'+ end=+'+ contains=tadsSpecial,tadsEmbedded
...
" no highlight for tadsEmbedded, so it prints as normal text w/in the string
  ...
  HiLink tadsTrebleDoubleString	tadsString
  HiLink tadsTrebleSingleString	tadsString
